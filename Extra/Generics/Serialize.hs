...
doLeaf :: Serialize a => p -> (String, String, String, Bool) -> (String, Fixity) -> a -> S1Result
doLeaf _p _ti _ci a = encode a

doList :: p -> (String, String, String, Bool) -> (String, Fixity) -> [ByteString] -> S1Result
doList _ _ _ [] = mempty
doList _ _ _ xs = mconcat xs

doTuple :: p -> (String, String, String, Bool) -> (String, Fixity) -> [ByteString] -> S1Result
  ...
  r@Counts{..} <-
    runTestTT
      (TestList
       ...
       , let x = ([1] :: [Int]) in TestCase (assertEqual "T8" "[1]" (myencode x))
